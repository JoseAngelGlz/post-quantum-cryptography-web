...
export const useAnalytics = () => {
  ...
  const sectionCompleted = (routeId: string, sectionIndex: number, timeSpent: number, sectionTitle?: string) => {
  ...
  const questionAnswered = (routeId: string, quizId: string, questionIndex: number, isCorrect: boolean) => {
  ...
  const quizAbandoned = (routeId: string, quizId: string, currentQuestion: number, total: number) => {
  ...
  const feedbackSent = (routeId: string, difficulty: number, clarity: number, recommend: number, hasComment: boolean) => {
